...
class M4Fog(data.Dataset):
    ...
    def __init__(self, root, split, get_name=False):
        ...
        # H8 mean and std
        # self.mean = torch.FloatTensor([123.89, 116.86, 104.82]).view(-1, 1, 1)
        # self.std  = torch.FloatTensor([32.43, 35.77, 44.36]).view(-1, 1, 1)
        # FY4A mean and std
        # self.mean = torch.FloatTensor([59.52, 60.17, 72.65]).view(-1, 1, 1)
        # self.std  = torch.FloatTensor([53.61, 52.66, 51.35]).view(-1, 1, 1)
        # H8+FY4A mean and std
        self.mean = torch.FloatTensor([92.28, 88.95, 89.02]).view(-1, 1, 1)
        self.std  = torch.FloatTensor([55.28, 55.54, 50.94]).view(-1, 1, 1)
